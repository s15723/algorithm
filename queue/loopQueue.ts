...
export default class LoopQueue<T> implements Queue<T> {
    ...
    dequeue(): T {
        ...
        this.size--;
        if (this.size === Math.ceil(this.getCapacity() / 4) && this.getCapacity() / 2 !== 0) {
            this.resize(Math.ceil(this.getCapacity() / 2));
        }
    ...
    toString() {
        let str = `Queue: size = ${this.size}, capacity = ${this.getCapacity()} `;
        str += 'front [';
